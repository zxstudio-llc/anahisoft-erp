...
import { Plus, Minus, Save, Send, Eye, ChevronLeft, Search, Package, Settings, Trash2, Calculator, User, CreditCard, ShoppingCart } from "lucide-react"
...
import { CreateInvoiceData, Customer, Product } from "@/common/interfaces/tenant/sales.interface"
...
export default function CreateInvoice({ clients = [], products = [] }: CreateProps) {
  ...
  // Campos adicionales específicos de factura
  ...
  // Función para generar el secuencial - MANTENER ESTA PARTE IGUAL
  const generateNextSequential = async () => {
    try {
      const response = await Api.get(`/v1/sales/next-sequential`, {
        params: {
          document_type: formData.document_type,
          establishment_code: additionalFields.establishment_code || '001',
          emission_point: additionalFields.emission_point || '001'
        }
      });

      const nextSequential = response.data.data.sequential;
      ...
      console.error('Error al obtener el próximo secuencial:', error);
      toast.error('Error al obtener el número de secuencial');
      setAdditionalFields(prev => ({
        ...prev,
        sequential: ''
      }));
  ...
  // Calculate totals - Adaptada para usar la misma estructura que POS
  ...
  // Funciones para manejar productos - Adaptadas para usar la misma estructura que POS
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Preparar datos para enviar (combinar formData y additionalFields)
      // Añadir el campo status para asegurar que se emita directamente
      const dataToSend = {
        ...formData,
        ...
        status: 'issued' // ← ESTE ES EL CAMPO CLAVE QUE FALTABA
      };

      const response = await Api.post('/v1/sales', dataToSend);

      toast.success('¡Factura emitida exitosamente!', {
        id: 'sale-processing',
        description: 'La factura ha sido emitida y registrada correctamente'
      });
      ...
      let errorMessage = 'Error desconocido al procesar la factura';
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      // LIMPIAR CAMPOS IGUAL QUE AL EMITIR FACTURA
      setFormData({
        customer_id: '',
        products: [],
        document_type: '01',
      ...
      toast.info('Formulario limpiado', {
        description: 'Puedes crear una nueva factura'
      });
  ...
  // Generar secuencial cuando cambien los campos relevantes
  useEffect(() => {
    generateNextSequential();
  }, [formData.document_type, additionalFields.establishment_code, additionalFields.emission_point]);
      ...
      <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Crear Factura</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={additionalFields.establishment_code || '001'}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="emissionPoint"
                      value={additionalFields.emission_point || '001'}
                      onChange={(e) => setAdditionalFields({
                        ...additionalFields,
                  ...
                  <div className="space-y-2 col-span-2">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        placeholder="Cargando número..."
                      />
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Próximo secuencial: {additionalFields.sequential ? `basado en último registro (${formatInvoiceNumber(
                        additionalFields.establishment_code,
                        additionalFields.emission_point,
                        additionalFields.sequential
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  ...
                  <Button onClick={() => console.log('Agregar item manual')} variant="outline">
